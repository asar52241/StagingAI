...
- Application servers handle images in memory and forward them to OpenAI; they do not persist image bodies to PostgreSQL or disk. Provider retention policies are separate from application storage.
- PostgreSQL (Neon) stores invoice ID, amount, count, owner nonce, confirmation/expiry and SHA-256 fingerprints/attempt state. Orders become inaccessible after seven days. During subsequent writes, each active instance deletes up to 1,000 expired records at most once per minute; physical deletion can happen later than expiry, and provider backups have their own retention. No source image, mask, result, filename, bank account or full Robokassa XML is stored there.
