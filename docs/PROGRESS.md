...
Исторический трекер предыдущей реализации. После объединения веток заказы и лимиты переведены на Neon/PostgreSQL, а результаты восстанавливаются из браузера. Указанные ниже настройки Upstash больше не применяются. Актуальные настройки и проверки: [README](../README.md) и [аудит](security-audit-2026-09-10.md).
